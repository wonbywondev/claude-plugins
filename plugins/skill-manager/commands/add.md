...
3. For each confirmed skill: **digest** (read body once via `sm_digest_prompt`, write a 1-2 line purpose-digest) → **dedup** (`sm_prefilter` over `sm_dedup_corpus` — the *whole* central repo, registry digests overlaid on catalog descriptions; empty = no dup, no LLM; else judge digests, subagent for ambiguous; `sm_shared_keywords` for trigger collisions → routing note).
