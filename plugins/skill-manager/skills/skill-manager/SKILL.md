...
`source ${CLAUDE_PLUGIN_ROOT}/lib/common.sh` then `lib/{fetch,select,digest,dedup,audit,place,register,link}.sh`. Paths via `SKILLS_REPO` / `GLOBAL_SKILLS_DIR` / `SKILL_MANAGER_HOME` (env override).
