...
0. **인박스 승격** (`wiki-manager` 위임) — `bash <wiki-manager>/scripts/wiki-inbox.sh list`로 `INSIGHT/_inbox.md` 후보 확인 → **소크라테스 게이트**(각 후보에 "모순/근거약함/더 일반화?" 되물어 다듬은 것만) → 렌즈로 **DECISION/INSIGHT/knowledge 풀 노트로 승격**(작성·링크) 또는 **폐기**. ⚠ 인박스는 세션 공유라 **이번 세션 후보만** 승격하고, 전체 `clear` 대신 **승격한 줄만 제거**(다른 세션 후보 보존). (없으면 통과.) 컨벤션은 wiki-manager 단일 소스.
